...
export function initMockData() {
  ...
  serviceNames.forEach((name) => {
    ...
    // generate 60 metric points 1 minute apart
...
export function getMetrics(id: string, rangeSec?: number): MetricPoint[] {
  const all = state.metrics[id] || [];
  if (rangeSec) {
    const cutoff = Date.now() - rangeSec * 1000;
    return all.filter((m) => (typeof m.timestamp === "number" ? m.timestamp : new Date(m.timestamp).getTime()) >= cutoff);
  }
  return [...all];
}
...
function paginate<T>(items: T[], page = 1, limit = 20): {items: T[]; total: number; page: number; limit: number} {

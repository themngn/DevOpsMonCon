        ...
        <div className="flex flex-col h-full h-[400px]">
            <ScrollArea className="h-full w-full rounded-md border p-4 bg-background">
                {loading ? (
                    ...
                    <div className="flex items-center justify-center p-8 text-muted-foreground">No recent logs found.</div>
                ) : (
                    <div className="space-y-2">
                        {logs.map((log) => {
                            ...
                            return (
                                <div key={log.id} className="flex flex-col sm:flex-row sm:items-start gap-2 py-1 border-b last:border-0 hover:bg-muted/50 transition-colors px-2">
                                    <div className="flex items-center gap-3 shrink-0 sm:w-48">
                                        ...
                                        <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded uppercase", getBadgeClass(log.level))}>
                                            {log.level}
                                        </span>
                                    </div>

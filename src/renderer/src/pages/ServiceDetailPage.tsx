...
export default function ServiceDetailPage() {
    ...
    const [actionStatus, setActionStatus] = useState<{ type: 'success' | 'error' | 'loading' | 'indeterminate', message?: string } | null>(null)
        ...
        <div className="flex flex-col gap-6 p-6 h-full">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-muted-foreground">Uptime: {Math.floor(service.uptime / 60)}m</span>
                </div>
                <div className="flex items-center gap-3">
                    {actionStatus && actionStatus.type !== 'indeterminate' && (
                        <span className={`text-sm ${actionStatus.type === 'error' ? 'text-red-500' : actionStatus.type === 'success' ? 'text-green-500' : 'text-blue-500'}`}>
                    ...
                    <Button variant="outline" className="border-amber-500 text-amber-500 hover:bg-amber-500 hover:text-white" onClick={() => setIsRestartOpen(true)}>
                        Restart
                    </Button>
